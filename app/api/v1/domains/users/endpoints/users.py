...
from app.api.common.responses import created, updated, not_found
...
def create_user(
    *,
    ...
    return created(obj_name="User", obj=user)
...
def update_user(
    *,
    ...
    if not user:
        return not_found(obj_name="User")
    user = user_service.update(db, db_obj=user, obj_in=user_in)
    return updated(obj_name="User", obj=user)
